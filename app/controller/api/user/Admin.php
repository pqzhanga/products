...
use app\controller\admin\system\config\Config;
...
class Admin extends BaseController
{
    ...
    public function setSoundCode(MerchantRepository $repository)
    {
        $sound_code = $this->request->post('sound_code');
        // $sound_code = $this->request->param('sound_code');
        $ret=Db::table('eb_merchant')->where(['mer_id' => $this->merId])->update(['sound_code' => $sound_code]);
    ...
    {
        $sound_code = Db::table('eb_merchant')->where(['mer_id' => $this->merId])->field('sound_code')->find();
        //return app('json')->success(compact('sound_code'));
        //return app('json')->success(compact('code'))
        //return $sound_code;
        //return 0;
        //return $this->merId;
        //var_dump("99889");
        return app('json')->success($sound_code);
    }
                ...
                //0成功，1处理中，3，审核拒绝（原因参考应答描述）
                    switch ($resdata['mchtStatus']){
                        ...
                        case '3':
                            ...
                            break;
                        default:
                            Log::write('-----error-----查询快付申请状态未知：'.var_export($resdata,true));
                            return app('json')->success('查询状态未知！');
                    }
            }
            Log::write('-----error-----查询快付申请状态错误2：'.var_export($resdata,true));
        }
        Log::write('-----error-----查询快付申请状态错误：'.var_export($encRespStr,true));
...
//        $qr_url = app()->make(QrcodeService::class)->getQRCodePath( $this->request->domain(true).'?mid='.$this->merId  , md5($this->request->domain(true).'?mid='.$this->merId) . '.jpg' )['thumb_path'];
        $qr_url =   $this->getcode(app()->make(QrcodeService::class)->getQRCodePath( $this->request->domain(true).'?mid='.$this->merId  , md5($this->request->domain(true).'?mid='.$this->merId) . '.jpg' )['filepath']);





        return app('json')->success(compact('mer_yes','mer_total','mer_money','mer_name','order', 'data','crate','qr_url','reg_url','scan_info','is_reg'));
    }
    ...
    private function  getcode($path){
        ...
        $config = array(
            ...
            'background'=>public_path().'file/bg.jpg'          //背景图
        );
