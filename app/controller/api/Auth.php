...
class Auth extends BaseController
{
    ...
    public function releaseUser(UserRepository $repository)
    {
        $user = $this->request->userInfo();
        if(!$user) return;
        $this->refreshUserTree($user->uid,'');
        $user->delete();
    ...
    private function refreshUserTree($uid,$retree_p ,$level = 0){
        ...
        foreach ($ch_users as $vv){
            $retree = $vv['retree'];
//            if($retree && strpos($retree , ",{$uid}," ) !== false){
//              if($level > 0)  $vv['retree'] = substr($retree,strpos($retree,  ",{$uid},"  ));
//              else  $vv['retree'] = str_replace(",{$uid},", ","  ,substr($retree,strpos($retree,  ",{$uid},"  )));
//            } else{
//                $vv['retree'] =   ",{$vv['uid']}," ;
...
//            }
    ...
    {
        $user = $this->request->userInfo()->hidden(['label_id', 'group_id', 'pwd', 'addres', 'card_id', 'last_time', 'last_ip', 'create_time', 'mark', 'status', 'spread_uid', 'spread_time', 'real_name', 'birthday']);
        $user->append(['service', 'total_collect_product', 'total_collect_store', 'total_coupon', 'total_visit_product', 'total_unread', 'total_recharge','card_id','idcardFront','idcardBack','certifyState']);
        $data = $user->toArray();
        $data['total_consume'] = $user['pay_price'];
        $data['extension_status'] = systemConfig('extension_status');

        $data['mersub'] =  $this->request->domain(true) . systemConfig('mersub');
        return app('json')->success($data);
    }

    public function cardInfoUpdate()
    {
        ...
        Db::name('user')
            ...
            ->update([
                ...
                'idcardFront'=>$request->param('idcardFront'),
                'idcardBack'=>$request->param('idcardBack'),
                'certifyState'=>0
            ]);
        ...
        return app('json')->success('资料提交成功！');
    }
...
//        }
    ...
    public function verify2(UserAuthValidate $validate)
    {
       $user =  $this->request->userInfo();
       if(!$user || !$user->phone)
           return app('json')->fail('请先绑定电话号码！');
        $data = $this->request->params(['phone', 'code', 'key', ['type', 'login']]);
        $data = [
            'phone'=>$user->phone,
            'type'=>'change_pwd',
...
//        }
    ...
    public function changePassword2(ChangePasswordValidate $validate, UserRepository $repository)
    {
        $user =  $this->request->userInfo();
        if(!$user || !$user->phone)
            return app('json')->fail('请先绑定电话号码！');

        $data = $this->request->params(['phone', 'sms_code', 'confirm_pwd', 'pwd']);
    ...
    public function register(UserRepository $repository){
        $data = $this->request->params(['account','pwd','nickname','spread_uid','mid']);
        if(!$data['spread_uid'] && $data['mid']){
            $mid  = intval($data['mid']);
            $mer = Db::table('eb_merchant')->find($mid);
            if($mer){
                $data['spread_uid'] = $mer['uid'];
            }
        }
        unset($data['mid']);

        $res = Db::name('user')->where('account|phone','=',$data['account'])->find();
        if($res === null){
            $data['pwd'] = $repository->encodePassword($data['pwd']);
            $res = Db::name('user')->strict(false)->insert($data);
            if($res===1) {
                $user =  $repository->get(Db::name('user')->getLastInsID());
                $user->brokerage_gongxian = 1000.00;
                $user->brokerage_duihuan = 10000.00;
                $user->avatar = '/static/images/f.png' ;
                $user->last_ip = app('request')->ip();
                if($user->uid == $user->spread_uid) $user->spread_uid = 0;
                if($user->spread_uid > 0){
                    $p_user = Db::table('eb_user')->find($user->spread_uid);
                    if($p_user){
                        $user->retree  = trim($p_user['retree'],',').','.$user->uid.',';
                        $user->layer  =   count(array_unique(array_filter(explode(',', $user->retree ))));
                        $user->spread_time = date('Y-m-d H:i:s');
                        Db::table('eb_user_bill')->where(['uid'=>$user->spread_uid])->inc('spread_count');
                    }else{
                ...
                $saveDate = [
                    ...
                    'number'=>$user->brokerage_gongxian,
                ...
                return app('json')->success('用户注册成功！');
            } else {
                return app('json')->success('用户注册失败！');
            }
        } else {
            return app('json')->success('用户已存在！');
        }
    }
}
