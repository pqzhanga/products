...
class Merchant extends BaseController
{
    ...
    public function lst()
    {
        ...
        $where = $this->request->params(['keyword', 'date', 'status', 'statusTag', 'is_trader', 'uaccount']);
    ...
    public function update($id, MerchantValidate $validate, MerchantCategoryRepository $merchantCategoryRepository)
    {
        ...
        if ($data['crate'] >30 || $data['crate'] < 3  )
            return app('json')->fail('让利比例介于3%-30%之间');


        if ( isset($data['district_id'] ) && $data['district_id']  )
        {
            if(is_array($data['district_id'])){

                list($provinceid,$cityid,$district) =  $data['district_id'];
...
//            $data['mer_address']  = $data['province'].$data['city'].$data['district'].' '.$data['mer_address'];
                ...
                $citys = app()->make(CityDao::class);
                $cityid =  $citys->getWhere(['city_id'=>$data['district_id']])['parent_id'];
                $provinceid =  $citys->getWhere(['city_id'=>$cityid])['parent_id'];

                $provinceid = intval($provinceid);
                $cityid = intval($cityid);
                $district = intval($data['district_id']);
...
//            $data['mer_address']  = $data['province'].$data['city'].$data['district'].' '.$data['mer_address'];
        ...
        if($old['uid'] != $data['uid']){
            Db::table('eb_user_relation')->where(['type'=>10,'type_id'=>$id,'uid'=>$old['uid']])->delete();
            Db::table('eb_store_service')->where(['uid'=>$old['uid'],'mer_id'=>$id])->delete();
            Db::table('eb_user_relation')->insert([
                'uid'=> $data['uid'] ,
                'type_id'=>$id,
                'type'=>10,
            ]);
            Db::table('eb_store_service')->insert([
                'uid'=> $data['uid'] ,
                'mer_id'=>$id,
                'status'=>1,
                'notify'=>1,
    ...
    {
        $data = $this->request->params([['category_id', 0], 'mer_name',  'sub_name', 'sub_mer' , 'crate' , 'commission_rate', 'real_name', 'mer_phone',  'protype', 'mer_keyword', 'mer_address',  ['uid', 0],'mark', ['sort', 0], ['status', 0], ['is_audit', 0], ['is_best', 0], ['is_bro_goods', 0], ['is_bro_room', 0], ['is_trader', 0],'district_id']);
        if (!$isUpdate)
