...
class Common extends BaseController
{
    ...
    public function main2()
    {
       ...
       $d1 = $UserExtractRepository->search(['status'=>'0'],1,1);
        $msgdata['tx'] = $d1['count'];

       $UserRepository = app()->make(UserRepository::class);
        $d2 = $UserRepository->getList([
            'user_type2'=>1
        ] , 1, 1);
        $msgdata['rz'] = $d2['count'];


        $MerchantRepository = app()->make(MerchantIntentionRepository::class);
        $d3 = $MerchantRepository->getList([
                'status'=>'0',
        ],1,1);
        $msgdata['sh'] = $d3['count'];

        return app('json')->success(compact('msgdata'));
    }



    public function ckcmpu2()
    {
        return app('json')->fail('功能已关闭,每日自动计算！');
        return ;
        exec('php /www/wwwroot/szys/think updateFourValue');
        return app('json')->fail('计算2已执行！');
    }

    public function ckcmpu()
    {
        return app('json')->fail('功能已关闭,每日自动计算！');
        return ;
        $res = Db::query('CALL  deal_auto_send()');
        return app('json')->fail('已执行！');
    }
