...
class Merchant extends BaseController
{
    ...
    {
        $data = $this->request->params(['mer_info', 'service_phone', 'mer_avatar', 'mer_banner', 'mer_state', 'mini_banner', 'mer_keyword', 'mer_address', 'long', 'lat','ccty','mer_cate']);
        $validate->check($data);
...
//mer_address
        ...
        if(!$data['mer_cate'] || !is_array($data['mer_cate']) || intval($data['mer_cate'][0]) <= 0 )      return app('json')->fail('请选择商户分类');
        $data['category_id'] = intval($data['mer_cate'][0]);
        unset($data['mer_cate']);
    ...
    public function info()
    {
        ...
        foreach ($lst as &$v){
            ...
            $v['value'] = $v['merchant_category_id'];
        }
        unset($v);
        return app('json')->success(array_merge($merchant->append(['merchantCategory'])->hidden(['mark', 'reg_admin_id', 'sort'])->toArray(),['citys'=>$citys->citys() ,'mer_cates'=>$lst]));
    }
