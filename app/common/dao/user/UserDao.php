...
class UserDao extends BaseDao
{
    ...
    {// like explode
        ...
        $query->when(isset($where['keyword']) && $where['keyword'], function (BaseQuery $query) use ($where) {
            ...
            return $query->where('User.uid', intval($where['uid']));
        ...
        })->when(isset($where['spread_uidss']) && $where['spread_uidss'] !== '', function (BaseQuery $query) use ($where) {
            $query->where('uid','<>',$where['spread_uidss'] );
            return $query->whereRaw('CONCAT(\',\',User.retree) LIKE \'%,' . $where['spread_uidss'] . ',%\'');
        ...
        })->when(isset($where['memberlevel2']) && $where['memberlevel2'] !== '', function (BaseQuery $query) use ($where) {
            return $query->where('User.memberlevel2', '>' , $where['memberlevel2']);
        })->when(isset($where['user_type2']) && $where['user_type2'] , function (BaseQuery $query) use ($where) {
            return $query->where('User.certifyState', 0 )->where('User.card_id', '<>' , '' );
        })->when(isset($where['memberlevel3']) && $where['memberlevel3'] !== '', function (BaseQuery $query) use ($where) {
            return $query->where('User.memberlevel3', '>' , $where['memberlevel3']);
        })->when(isset($where['agentLevel']) && $where['agentLevel'] !== '', function (BaseQuery $query) use ($where) {
            return $query->where('User.agentLevel', '>' , $where['agentLevel']);
...
//            $res = User::getDB()->where('phone', $account)->find();
//            if($res) return $res;
//        }
    ...
    public function getParInfo(array $spread_uid)
    {
        return User::getDB()->where('uid', $spread_uid)->find();
    }
    ...
    public function userOrderDetail($uid)
    {
        return User::getDB()->alias('A')
            ...
            ->field('A.mark,A.memberlevel3,A.memberlevel2,A.brokerage_duihuan,A.brokerage_price,A.brokerage_shuquan,A.brokerage_gongxian,A.agentlevel,A.uid,A.avatar,A.account,A.now_money,A.pay_price,A.pay_count, sum(B.pay_price + IFNULL(C.pay_price,0)) as total_pay_price, count(B.order_id) as total_pay_count')
            ->where('A.uid', $uid)
            ->find();
    }
