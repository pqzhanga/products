    ...
    public function search(array $where, $sysDel = 0)
    {
        ...
        $query->when(($sysDel !== null), function ($query) use ($sysDel) {
            ...
            ->when(isset($where['mer_name']) && $where['mer_name'] !== '', function ($query) use ($where) {
                $uids = Merchant::where('mer_name', 'like', "%{$where['mer_name']}%")  ->column('mer_id');
                $query->where('StoreOrder.mer_id', 'in', $uids);
            })
            ->when(isset($where['order_type']) && $where['order_type'] >= 0 && $where['order_type'] !== '', function ($query) use ($where) {
                $query->where('StoreOrder.order_type', $where['order_type']);
            })
            ...
            ->when(isset($where['status']) && $where['status'] !== '', function ($query) use ($where) {
                if ($where['status'] == -2)
                    $query->where('StoreOrder.paid', 1)->whereNotIn('status', [10, 11]);
            ...
            ->when(isset($where['zone_id']) && $where['zone_id'] !== '', function ($query) use ($where) {
                $query->where('StoreOrder.order_type_n', $where['zone_id']);
            })
            ...
            ->when(isset($where['take_order']) && $where['take_order'] != '', function ($query) use ($where) {
                $query->where('StoreOrder.order_type', 1)->whereNotNull('verify_time');
            })
            ...
            ->when(isset($where['verify_date']) && $where['verify_date'] !== '', function ($query) use ($where) {
                getModelTime($query, $where['verify_date'], 'StoreOrder.verify_time');
            })
            ->when(isset($where['order_sn']) && $where['order_sn'] !== '', function ($query) use ($where) {
                $query->where('StoreOrder.order_sn', 'like', '%' . $where['order_sn'] . '%');
            })
            ...
            ->when(isset($where['service_id']) && $where['service_id'] !== '', function ($query) use ($where) {
                $query->where('StoreOrder.service_id', $where['service_id']);
            })
            ->when(isset($where['username']) && $where['username'] !== '', function ($query) use ($where) {
                $uid = User::where('nickname', 'like', "%{$where['username']}%")
                    ...
                    ->column('uid');
                $query->where('StoreOrder.uid', 'in', $uid);
            })
            ->when(isset($where['keywords']) && $where['keywords'] !== '', function ($query) use ($where) {
                $query->where(function ($query) use ($where) {
                    $query->whereLike('StoreOrder.real_name|StoreOrder.user_phone|StoreOrder.order_sn', "%" . $where['keywords'] . "%");
                });
            })
            ->when(isset($where['reconciliation_type']) && $where['reconciliation_type'] !== '', function ($query) use ($where) {
                $query->when($where['reconciliation_type'], function ($query) use ($where) {
                    $query->where('StoreOrder.reconciliation_id', '<>', 0);
                }, function ($query) use ($where) {
                    $query->where('StoreOrder.reconciliation_id', 0);
                });
            })
            ->when(isset($where['goodsname']) && $where['goodsname'] !== '', function ($query) use ($where) {
                $product_ids = Product::where('store_name', 'like', "%{$where['goodsname']}%")
                    ->column('product_id');
               $query->leftJoin('eb_store_order_product SGOI', 'StoreOrder.order_id = SGOI.order_id')->where('SGOI.product_id','in',$product_ids);
            })->order('StoreOrder.pay_time desc , StoreOrder.create_time desc');
