...
class MerchantDao extends BaseDao
{
    ...
    {
        $query = Merchant::getDB()->alias('mer')
            ->when($is_del !== null, function ($query) use ($is_del) {
                $query->where('mer.is_del', $is_del);
            })
            ->when(isset($where['is_trader']) && $where['is_trader'] !== '', function ($query) use ($where) {
                $query->where('mer.is_trader', $where['is_trader']);
            })
            ->when(isset($where['is_best']) && $where['is_best'] !== '', function ($query) use ($where) {
                $query->where('mer.is_best', $where['is_best']);
            })
            ->when(isset($where['date']) && $where['date'] !== '', function ($query) use ($where) {
                getModelTime($query, $where['date']);
            })
            ->when(isset($where['protype']) && $where['protype'] !== '', function ($query) use ($where) {
                $query->where('mer.protype', $where['protype']);
            })
            ...
            ->when(isset($where['category_id']) && $where['category_id'] !== '', function ($query) use ($where) {
                is_array($where['category_id']) ? $query->whereIn('mer.category_id', $where['category_id']) : $query->where('mer.category_id', $where['category_id']);
            });

        if (isset($where['keyword']) && $where['keyword']) {
            if (is_numeric($where['keyword'])) {
                $query->whereLike('mer_phone', "%{$where['keyword']}%");
            } else {
                $query->whereLike('mer_name|mer_keyword', "%{$where['keyword']}%");
...
//                });
        ...
        if (isset($where['status']) && $where['status'] !== '')
            $query->where('mer.status', $where['status']);
        $order = $where['order'] ?? '';
