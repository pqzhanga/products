...
class StoreOrderRepository extends BaseRepository
{
    ...
    public function createOrder(User $user, int $pay_type, array $cartId, int $addressId, array $coupons, array $takes, array $mark, array $receipt_data)
    {
        ...
        foreach ($orderInfo as $k => $cartInfo) {
            ...
            $_order = [
                ...
                'stock_type'=>$cartInfo['list'][0]['product']['stock_type']  ,
            ];
     ...
     * @day 2020/10/22
     */
    public function pay(string $type, User $user, StoreGroupOrder $groupOrder, $return_url = '',$is_app = false,$fastpay_type = '')
    {
        $method = 'pay' . ucfirst($type);
        if (!method_exists($this, $method))
            throw new ValidateException('不支持该支付方式');
        return $this->{$method}($user, $groupOrder, $return_url,$is_app,$fastpay_type);
    }

    public function doordersplit($order){
        //分账
        ...
        $mchtCd = "MCHT048161635";//2021-6-22BYWH
        ...
        $sub_mer_info = Db::table('eb_merchant')->where(['mer_id'=>$order['mer_id']])->find();
        if(!$sub_mer_info || !$sub_mer_info['sub_mer']) return false;
        $sub_mer = $sub_mer_info['sub_mer'];
...
//
        ...
        //外部订单号
        $reqData["outOrderId"] = $order['fz_order']; // 此处仅做示例,故以时间戳为值oglOrdId
        $reqData["oglOrdId"] = $order['out_order_sn']; // 此处仅做示例,故以时间戳为值oglOrdId
        //交易金额, 单位：元
        $reqData["transAmt"] = $order['money'];//"0.1";
        //产品编号，统一下单：tran
        $reqData["proCd"] = "tran";
...
//            ]
        ...
        Log::write('分账--请求报文：' . json_encode($reqData),'notice');
        ...
        Log::write('分账--三方接口返回数据3：' . $respStr ,'notice');

        $respData = json_decode($respStr, true);

        if("0000" == $respData["respCode"] && $respData['respMsg'] == '成功' ){
    ...
    public function queryorderinfo($time,$ordersn){
        try {
//机构号：202010209693553
//机构密钥：L2DYNQ5YR9P532ZTX8WNTBWX
//商户号：MCHT965103250
...
//
            ...
            //商户编号
            $reqData["mchtCd"] = 'MCHT965103250';
            //原消费的外部订单号
            $reqData["oglOrdId"] = $ordersn;
            //原消费的交易日期
            $reqData["oglOrdDate"] = $time;


            Log::write('查询订单接口请求数据：' . json_encode($reqData),'notice');
            ...
            if("0000" == $respData["respCode"]){
                ...
                /* 交易异常 */
                Log::info('-------------error------ 查询订单接口获取最终状态:' . $respData["respMsg"]  );
                /*交易状态未知，请调查询接口获取最终状态*/
            }
        } catch (Exception $e){
            Log::info('-------------error------ 查询订单失败:' . var_export([$e->getMessage(), $e->getFile() . ':' . $e->getLine()], true));
        }
        return false;
    }
    ...
    //临时测试用h5代替小程序支付
    public function temppayyunfastpay_app_chongzhi($order_sn,$money){
        // var_dump($order_sn);
        // var_dump($money);
        // die;
        $typeField = "ORG";
...
//
        ...
        $reqData["appid"] = $APPID;//H5开启,如果是app支付要注释掉2021-7-4BYWH

        //外部订单号
        $reqData["outOrderId"] = $order_sn; // 此处仅做示例,故以时间戳为值
        //交易金额, 单位：元
        $reqData["transAmt"] = $money;//"0.1";
        // $reqData["transAmt"] =  "0.01";
        //产品编号，统一下单：tran
        $reqData["proCd"] = "tran";
        //费率通道，公众号支付：1，生活号支付：1，小程序支付：7，H5支付/APP支付：7
...
//        /pages/order_pay_status/index?order_id=37
        ...
        if($respData  && "0000" == $respData["respCode"] && $respData['orderCd']   &&  $respData['sign'] ){
            return app('json')->status('yunfastpay', '请支付！', ['orderCd' => $respData['orderCd'] , 'sign' => $respData['sign'] , 'order_id' => 'CZ2021061223002416235100248640','url' => mb_ereg_replace('%23','#',$respData["qrData"]) ,'qrdata'=> mb_ereg_replace('%23','#',$respData["qrData"])  ]);
        }
        else {
    ...
    //public  function payyunfastpay_app_chongzhi(User $user, StoreGroupOrder $groupOrder,$fastpay_type ){
    public function payyunfastpay_app_chongzhi($order_sn,$money){
        // var_dump($order_sn);
        // var_dump($money);
        // die;
        $typeField = "ORG";
...
//        /pages/order_pay_status/index?order_id=37
    ...
    //线上商城h5支付 //目前没用上 2021-7-4
    public  function tamppayyunfastpay_app(User $user, StoreGroupOrder $groupOrder,$fastpay_type ){
...
//        /pages/order_pay_status/index?order_id=37
        ...
        if($respData  && "0000" == $respData["respCode"] && $respData['orderCd']   &&  $respData['sign'] ){
            return app('json')->status('yunfastpay', '请支付！', ['orderCd' => $respData['orderCd'] , 'sign' => $respData['sign'] , 'order_id' => $groupOrder['group_order_id'],'url' => mb_ereg_replace('%23','#',$respData["qrData"]) ,'qrdata'=> mb_ereg_replace('%23','#',$respData["qrData"])  ]);
        }
        else {
            return app('json')->fail('网关错误请稍后再试！');
        }
    ...
    public  function payyunfastpay_app(User $user, StoreGroupOrder $groupOrder,$fastpay_type ){
...
//        /pages/order_pay_status/index?order_id=37
    ...
    public function payyunfastpay(User $user, StoreGroupOrder $groupOrder,$return_url,$is_app,$fastpay_type){
        Log::write('快付订单数据'.$groupOrder,'notice');


        if($is_app && $fastpay_type == 'wxpay'){
            Log::write('快付订单数据'.$groupOrder,'notice');
            return $this->payyunfastpay_app( $user,  $groupOrder,$fastpay_type);
        }
...
//                }
                ...
                if($order->stock_type && $order->order_type == 0  ){
                    ...
                    foreach ($order->orderProduct as $vvv){
                        ...
                        $attrValueRepository->descStock($vvv['product_id'], $vvv['product_sku'], $vvv['product_num']);
                        $productRepository->descStock($vvv['product_id'], $vvv['product_num'] );
...
//        }
    ...
    public function takeOrder($id, ?User $user = null)
    {
        ...
        app()->make(MerchantReconciliationRepository::class)->create2($order->mer_id);
...
//        }
    ...
    public function adminGetList(array $where, $page, $limit)
    {
        ...
        $list = $query->page($page, $limit)->select()->each(function ($item){
            $item['order_zone'] = '';
            switch ($item['order_type_n']){//1、普通区 3、促销区4、兑换区5、厂供区 6 创始专享区   order_zone
                case 1:
                    $item['order_zone'] = '普通区';
                    break;
                case 3:
                ...
                case 5:
                    ...
                    break;
                case 6:
                    $item['order_zone'] = '创始专享区';
                    break;
            }
        });

        $productId = $this->dao->search($where)->where($this->getOrderType($status))->column('StoreOrder.order_id');
        $make = app()->make(StoreRefundOrderRepository::class);
        $presellOrderRepository = app()->make(PresellOrderRepository::class);
        $orderRefund = $make->refundPirceByOrder($productId);
        $all = $this->dao->search($where)->where($this->getOrderType($status))->where('paid', 1)->count();
