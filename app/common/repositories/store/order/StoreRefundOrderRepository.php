...
use crmeb\services\PayfastService;
...
class StoreRefundOrderRepository extends BaseRepository
{
     ...
     * @day 2020/6/17
     */
    public function refund(StoreOrder $order, $productId, $num, $uid, array $data,$is_super_admin = false)
    {
        $orderId = $order->order_id;
        //TODO 订单状态生成佣金
        $product = app()->make(StoreOrderProductRepository::class)->userRefundProducts([$productId], $uid, $orderId,$is_super_admin);
        if (!$product || !isset( $product[0] ))
            throw new ValidateException('当前商品无可退额度，请选择正确的退款商品');
        $product = $product[0];
     ...
     * @day 2020-06-13
     */
    public function agree(int $id, array $data, int $adminId = 0)
    {
        //已退款金额
        ...
        Db::transaction(function () use ($id, $data, $adminId,$_refund_price) {
            $res = $this->getWhere(['refund_order_id' => $id], '*', ['refundProduct.product']);
            $this->getProductRefundNumber($res, 1);//退款货物标记
            if ($res['refund_type'] == 1) {
                //TODO 退款单同意退款
                $refund = $this->doRefundPrice($id,$_refund_price,$adminId);
                $data['status'] = 3;
...
///退回兑换值
        $order = app()->make(StoreOrderRepository::class)->getOne($refundOrder['order_id'],0);
        if(in_array($order['order_type_n'],[4,5]) && $order['pay_duihuan'] > 0){
            $user = Db::name('user')->where('uid',$order['uid'])->inc('brokerage_duihuan', $order['pay_duihuan'] )->update();
            $saveDate = [
                'uid' => $order['uid'],
                ...
                'balance'=>bcadd($user['brokerage_duihuan'],   $order['pay_duihuan']  ,2),
                'mark'=>'订单退款，兑换值增加'. $order['pay_duihuan'] .' 备注:'.$order['order_sn'] ,
     ...
     * @day 2020-06-13
     */
    public function doRefundPrice($id,$refundPrice,$adminId = 0)
    {
            ...
            if($item['data']['pay_price'] > 0 && $item['data']['refund_price'] > 0){
                ...
                if($item['type'] == 0){
                    ...
                    if(in_array($item['type'],[4,5])) {
                        $item['data']['adminId'] = $adminId;
                        $server =  AlipayService::create();
                    }

                    if(in_array($item['type'],[6])) {
                        $item['data']['adminId'] = $adminId;
                        $server =  PayfastService::create();
                    }
