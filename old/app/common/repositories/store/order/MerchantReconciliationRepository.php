...
class MerchantReconciliationRepository extends BaseRepository
{
    ...
    public function create2( $mer_id)
    {
        $id = $mer_id;
        $data = [
            'adminId'=>0,
        ...
        if($data['order_type']) { //全选
            ...
            )->whereIn('status', [2,3])->whereNotIn('order_id', $data['order_out_ids'])->column('order_id');
        }

        if(is_array($order_ids) && (count($order_ids) < 1)  ){
            return false;
        }
        $compute = $this->compute($id,$order_ids,[]);
        $createData = [
            'status'        => 0,
            'mer_id'        => $id,
            'is_accounts'   => 0,
        ...
        Db::transaction(function()use($order_ids,$orderMake,$refundMake,$createData){
...
//            $refundMake->updates($refund_order_ids,['reconciliation_id' => $res['reconciliation_id']]);
            $this->reconciliationOrder($order_ids,[],$res['reconciliation_id']);

            //ok
            $this->switchStatus($res['reconciliation_id'],[
                'status'=>2,
                'mer_admin_id'=>0
            ]);
            ...
            $this->switchStatus($res['reconciliation_id'],[
                'is_accounts'=>1,
                'accounts_time'=>date('Y-m-d H:i:s',time())
            ]);
...
//            $_order_extension = bcadd($order['extension_one'],$order['extension_two'],3);
            $_order_extension = $order['rangli_coin'];
...
//            $_order_rate = bcmul(bcsub($order['pay_price'],$_order_extension,3),$commission_rate,3);//total_price
//            $_order_rate = bcsub($order['pay_price'],$_order_extension,3);//total_price
            $_order_rate = 0;
...
//            ], $res['mer_id']);
            ...
            bcscale(3);
            $res2 = Db::table('eb_merchant_bill')->save([
                'mer_id'=>$res['mer_id'],
            ...
            if($res2){
                ...
                $mer->money = bcadd($mer->money , $res['price'] );
                $mer->save();
