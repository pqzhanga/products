...
class UserDao extends BaseDao
{
    ...
    {// like explode
        ...
        $query->when(isset($where['keyword']) && $where['keyword'], function (BaseQuery $query) use ($where) {
            ...
            return $query->where('User.certifyState', 0 );
    ...
    public function userOrderDetail($uid)
    {
        return User::getDB()->alias('A')
            ...
            ->field('A.memberlevel3,A.memberlevel2,A.brokerage_duihuan,A.brokerage_price,A.brokerage_shuquan,A.brokerage_gongxian,A.agentlevel,A.uid,A.avatar,A.account,A.now_money,A.pay_price,A.pay_count, sum(B.pay_price + IFNULL(C.pay_price,0)) as total_pay_price, count(B.order_id) as total_pay_count')
            ->where('A.uid', $uid)
            ->find();
    }
