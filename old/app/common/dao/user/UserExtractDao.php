...
class UserExtractDao extends  BaseDao
{
    ...
    public function search(array $where)
    {
        if(isset($where['wechat']) && $where['wechat'] != '') {
            $query = model::alias('UserExtract')->hasWhere('user',function ($query)use($where){
                $query->where('UserExtract.nickname',"%{$where['wechat']}%");
            });
        }else{
            $query = model::alias('UserExtract');
            $query->field('UserExtract.*')->join('user user', 'user.uid = UserExtract.uid ', 'LEFT');
        }
       ...
       $query->when(isset($where['uid']) && $where['uid'] != '',function($query)use($where){
            $query->where('UserExtract.uid',$where['uid']);
               ...
               $query->where('UserExtract.use_type',$where['use_type']);
           })->when(isset($where['extract_type']) && $where['extract_type'] != '',function($query)use($where){
            $query->where('UserExtract.extract_type',$where['extract_type']);
        })->when(isset($where['keyword']) && $where['keyword'] != '',function($query)use($where){
           $query->whereLike('UserExtract.uid|UserExtract.bank_code|UserExtract.alipay_code|UserExtract.wechat|user.account',"%{$where['keyword']}%");
