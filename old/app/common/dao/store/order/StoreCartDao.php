...
class StoreCartDao extends BaseDao
{
    ...
    public function cartIbByData(array $ids, int $uid, ?int $cityId, bool $confirm = true)
    {
        return StoreCart::getDb()->where('uid', $uid)->with(['product' => function (Relation $query) use ($cityId) {
            $query->field('product_id,image,cate_parid,rangli,duihuan,gongxian,store_name,is_show,status,is_del,unit_name,price,mer_status,temp_id,give_coupon_ids,is_gift_bag,is_used,product_type,old_product_id');
            if ($cityId) {
