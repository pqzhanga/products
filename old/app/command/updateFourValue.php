...
class updateFourValue extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        bcscale(2);
        $list = Db::table('eb_user')->chunk(500,function($users) use($ctime){
            foreach($users as $user){
                if($user['sign_time']!==null && ( date('Y-m-d',strtotime($user['sign_time']) + 86400) == date('Y-m-d') ) && $user['brokerage_gongxian'] >= 100){
